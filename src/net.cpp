...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Bits " + FormatFullVersion();
...
static const char *strTestNetDNSSeed[][2] = {
    {"bitstools.com", "testnet-seed.bitstools.com"},
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Bits is probably already running."), addrBind.ToString().c_str());
        else
