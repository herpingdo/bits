...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Bits: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Bits Signed Message:\n";
...
{
    // Bits: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 30 * COIN; // Block value

    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 840000); // Bits: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Bits: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Bits: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bits: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bits: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Bits: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x5f9ea27afc2a5aa8f3854c36944634eb4bd849984f17a6526ed96c80106dda66");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Philippines typhoon: Mangrove trees seen as way to limit storm damage";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x6874f38911241e07e36a8ec1e67696d26065d5c0cc2d28522cbac54014d44810"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Bits: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitsMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitsMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BitsMiner(CWallet *pwallet)
{
    printf("BitsMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bits-miner");
    ...
    try { loop {
        ...
        printf("Running BitsMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitsMiner, pwallet));
}
